...
useEffect(() => {
  if (successMessage) {
    const timer = setTimeout(() => {
      setSuccessMessage(""); // ✅ Clear after 3 seconds
    }, 3000);
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              disabled={!editing}
              className={`${editing ? "focus:outline-none focus:ring-sunPurple focus:border-sunPurple border rounded-md":"bg-transparent border-b border-sunPurple"} mt-1 block w-full px-3 py-2`}
            />
          </div>
          ...
          <div className="flex justify-between mt-6">
            {editing ? (
              ...
              <button
                ...
                className="sm:bg-sunPurple text-sunPurple hover:underline font-semibold cursor-pointer sm:text-white sm:px-7 sm:py-2 sm:rounded-md sm:hover:text-gray-200  sm:hover:bg-sunPurple/80 transition duration-300 ease-in-out"
              >
                Edit Name
              </button>
            )}
            ...
            onClick={() => setShowModal(true)}
              disabled={deleting}
              className="sm:bg-red-500 font-semibold cursor-pointer sm:text-white sm:px-5 sm:py-2 sm:rounded-md sm:hover:text-gray-200 sm:hover:bg-red-700 transition duration-300 ease-in-out text-red-500 hover:underline"
            >
