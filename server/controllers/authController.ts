...
import { generateResetToken, generateToken, generateRefreshToken } from "../utils/tokenService"
...
export const registerUser = async (req:Request, res: Response) => {
    ...
    const cleanUser = await User.findById(newUser._id)
      .select("-password -otp -otpExpires -resetToken -__v -resetTokenExpires"); // exclude sensitive/internal fields
    //   .lean(); return plain JS object

    // ✅ Send response with clean data
    res.status(201).json({
      message: "Registration successful. OTP sent! Check your email inbox or spam folder.",
      data: cleanUser,
    });
...
export const verifyOTP = async(req: Request, res:Response) => {
    try {
        ...
        if (user.otpExpires && user.otpExpires < new Date()) {
            return res.status(400).json({message: "OTP has expired. Please request a new OTP.", code: "OTP_EXPIRED"}); // code needed to display button for resend only when it is a case of expired otp.
        }
    ...
    res.status(500).json({ message: "Verification failed", error: String(error) });
  }
    }
}
...
export const loginUser = async (req:Request, res:Response) => {
    try {
        ...
        if(!user.isVerified) {
            return res.status(403).json({message: "User not verified. Please check your email for OTP."});
        }
...
export const resendOTP = async(req:Request, res:Response) => {
    try{
        ...
        res.status(200).json({message: "New OTP sent to email"});
         ...
         if (error instanceof Error) {
    res.status(500).json({ message: "Failed to resend OTP. Please try again", error: error.message });
  } else {
    res.status(500).json({ message: "Failed to resend OTP. Please try again", error: String(error) });
  }
    }
}
...
export const forgotPassword = async(req: Request, res: Response) => {
    try {
        ...
        res.status(200).json({message: "Password reset link sent!. Check your inbox or spam folder."});
    } catch (error) {
        if (error instanceof Error) {
    res.status(500).json({ message: "Failed to send reset link", error: error.message });
  } else {
    res.status(500).json({ message: "Failed to send reset link", error: String(error) });
  }
    }
}
...
export const resolveResetCode = async (req: Request, res: Response) => {
  ...
  return res.status(200).json({ email: user.email, token: user.rawToken});
      ...
      if (error instanceof Error) {
        res.status(500).json({ message: "Invalid or expired reset code", error: error.message });
  } else {
    res.status(500).json({ message: "Invalid or expired reset code", error: String(error) });
  }
  }
...
export const refreshToken = (req: Request, res: Response) => {
  ...
  try {
    const decoded = jwt.verify(token, REFRESH_SECRET!) as unknown as {id: string};
    ...
    if (error instanceof Error) {
        res.status(500).json({ message: "Invalid refresh token", error: error.message });
  } else {
    res.status(500).json({ message: "Invalid refresh token", error: String(error) });
  }
  }
